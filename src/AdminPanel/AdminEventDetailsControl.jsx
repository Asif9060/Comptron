...
const AdminEventDetailsControl = () => {
  ...
  const handleEdit = (event) => {
    ...
    // Note: You'll have to handle images separately for edit (show previews if needed)
  };
  ...
  return (
    <div className="p-8 flex flex-col translate-y-[70rem]  ">
      ...
      <div className="flex flex-col items-center border border-[#15A6E1] p-4 rounded-3xl shadow-md mb-8">
        ...
        <form className="flex flex-col gap-4 w-full max-w-md" onSubmit={handleSubmit} encType="multipart/form-data">
          <input
          ...
          />
          <button className="button0 bg-emerald-500 text-white p-2 rounded-lg" type="submit">
      ...
      {/* Event List */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> {/* grid-cols-1 md:grid-cols-2 lg:grid-cols-3*/}
        {events.map((event) => (
          <div key={event._id} className="border border-gray-300 rounded-lg p-4 shadow-md flex flex-col items-center">
