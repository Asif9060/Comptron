...
const AdminUsersPage = () => {
   ...
   const handleSave = async () => {
      try {
         ...
         setUsers((prevUsers) =>
            prevUsers.map((user) => (user.customId === editUserId ? updatedUser : user))
         );
   ...
   const handleBulkValidation = async () => {
      ...
      for (const customId of selectedUsers) {
         try {
            const res = await fetch(
               `https://comptron-server-2.onrender.com/api/users/validate/${customId}`,
               {
                  method: "PUT",
               }
            );
      ...
      if (errorCount === 0) {
         ...
         setBulkValidationMessage(
            `Validated ${successCount} users. Failed to validate ${errorCount} users.`
         );
                           ...
                           <tr
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                 ...
                                 <button
                                    ...
                                    onClick={() =>
                                       window.open(`/profile/${user.customId}`, "_blank")
                                    }>
                                    View
                                 </button>
                              </td>
